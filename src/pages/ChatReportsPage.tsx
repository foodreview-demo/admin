...
import type { ChatReport, ReportStatus } from '../types';
import {
  MessageSquare,
  CheckCircle,
  XCircle,
...
const statusConfig: Record<ReportStatus, { label: string; color: string; icon: typeof MessageSquare }> = {
...
export function ChatReportsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['chat-reports', statusFilter, page],
    queryFn: () => adminApi.getChatReports(statusFilter || undefined, page, 10),
  });

  const processMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: { action: 'RESOLVE' | 'REJECT'; adminNote?: string } }) =>
      adminApi.processChatReport(id, data),
    onSuccess: () => {
  ...
  const reports: ChatReport[] = data?.data?.content || [];
  const totalPages = data?.data?.totalPages || 0;

  const handleProcess = (action: 'RESOLVE' | 'REJECT') => {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">채팅 신고 관리</h1>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-gray-500">채팅 신고 내역이 없습니다</p>
        </div>
      ) : (
        <>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">신고 대상</th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">메시지 내용</th>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <h2 className="text-xl font-bold">채팅 신고 상세 #{selectedReport.id}</h2>
            </div>

            <div className="p-6 space-y-6">
              {/* 신고 대상 정보 */}
              <div>
                <h3 className="font-medium text-gray-700 mb-2">신고 대상</h3>
              ...
              {selectedReport.messageContent && (
                <div>
                  <h3 className="font-medium text-gray-700 mb-2">신고된 메시지</h3>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <p className="text-gray-800">{selectedReport.messageContent}</p>
                  </div>
                </div>
              )}
